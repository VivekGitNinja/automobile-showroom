...
function InventoryContent({ initial }: { initial?: InitialInventory }) {
  ...
  // Server-provided first page: used once when the URL has no filters, so
  // the listing grid is present in the server-rendered HTML for crawlers.
  const usedInitial = useRef(false)
  useEffect(() => {
    if (
      initial && initial.applies && !usedInitial.current &&
      selectedMake === 'All' && !selectedBrand && !debouncedSearch &&
      selectedFuel === 'All' && selectedTransmission === 'All' &&
      !selectedMinYear && selectedSort === '-createdAt' && page === 1
    ) {
      usedInitial.current = true
      setVehicles(initial.data)
      setTotalPages(initial.totalPages)
      setTotalVehicles(initial.total)
      setLoading(false)
      return
    }
...
export default function InventoryClient({ initial }: { initial?: InitialInventory }) {
  return (
    <Suspense fallback={
      ...
      <InventoryContent initial={initial} />
    </Suspense>
  )
}
