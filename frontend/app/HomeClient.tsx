...
interface HomeClientProps {
  initialFeatured: Vehicle[]
  initialArrivals: Vehicle[]
  initialJournals: Journal[]
  initialTotal: number | null
}

export default function HomeClient({ initialFeatured, initialArrivals, initialJournals, initialTotal }: HomeClientProps) {
  const { toast } = useToast()
  const hasServerData = initialTotal !== null || initialFeatured.length > 0 || initialArrivals.length > 0

  const [featuredVehicles, setFeaturedVehicles] = useState<Vehicle[]>(initialFeatured)
  const [latestArrivals, setLatestArrivals] = useState<Vehicle[]>(initialArrivals)
