...
router.post('/sell-car', leadLimiter, async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const { company_website: _, ...submissionData } = data
    ...
    try {
      await notificationQueue.add('sell_car_notification', {
        ...
        text: [
          ...
          data.imageUrls && data.imageUrls.length > 0 ? `Images (${data.imageUrls.length}):\n${data.imageUrls.join('\n')}` : null,
        ...
        html: [
          ...
          data.imageUrls && data.imageUrls.length > 0 ? `<b>Photos attached:</b> ${data.imageUrls.length}<br>` : '',
...
router.post('/upload', leadLimiter, upload.array('files', 12), async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const urls = await Promise.all(
      req.files.map(async (file) => {
        ...
        const uploadResult = await storageService.uploadFile({
          filename: `sell-car-uploads/${Date.now()}-${safeName}`,
          buffer: file.buffer,
          mimeType: file.mimetype,
        })
        const rawUrl = (uploadResult as any).url || (uploadResult as any).fileUrl || ''
        return rawUrl.startsWith('http') ? rawUrl : `${baseUrl}${rawUrl.startsWith('/') ? '' : '/'}${rawUrl}`
      })
    )

    res.status(201).json({ urls, data: { urls } })
  } catch (err: any) {
    next(err)
  }
})
